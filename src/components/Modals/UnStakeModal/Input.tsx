...
}:
{
  shares: string,
  unStakeValue: string,
  setUnStakeValue: React.Dispatch<React.SetStateAction<string>>,
}) {
  ...
  const setMax = () => {
    ...
    if (maxShares.gte(0)) {
      setUnStakeValue(removeTrailingZeros(maxShares.toFixed()));
    }
  };
  ...
  return (
    <Block>
      ...
      <InputContainer>
        ...
        />
        <SharesContainer>
          {getUpperCase(t('unStakeModal.shares'))}
        </SharesContainer>
      </InputContainer>
    </Block>
  );
